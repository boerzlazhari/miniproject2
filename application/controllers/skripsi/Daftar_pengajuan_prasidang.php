...
class Daftar_pengajuan_prasidang extends CI_Controller {
	...
	{	
		if ($this->session->userdata('user_level') == 5) {
			$data_pengajuan = $this->pra_sk_pengajuan_m->get_datatable(array(1));
		}
		elseif ($this->session->userdata('user_level') == 3) {
			$data_pengajuan = $this->pra_sk_pengajuan_m->get_datatable(array(2));
		}
		elseif ($this->session->userdata('user_level') == 2) {
			$data_pengajuan = $this->pra_sk_pengajuan_m->get_datatable(array(3,4,5));
		}
		else {
		...
		$data = array(
			...
			'header'         => 'Daftar Pra Sidang',
			'header_child'   => 'Skripsi STMIK Bandung',
			'view'           => 'sk/prasidang/daftar_prasidang', 
	...
	{
		$mahasiswa = $this->pra_sk_pengajuan_m->get_data_prasidang($id);

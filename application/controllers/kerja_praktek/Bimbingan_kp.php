...
class Bimbingan_kp extends CI_Controller {
	...
	public function index()
	{
		if($this->session->userdata('user_level') == 7){
			$data_kp = $this->kp_pengajuan_m->get_where(array('mahasiswa_id' => $this->session->userdata('id'), 'status' => 2), true);
			$data_bimbingan = $this->kp_bimbingan_m->get_where(array('kp_id' => $data_kp->id));
			$count = count($data_bimbingan);
		}else
		{
			$data_kp = array();
			$data_bimbingan = $this->kp_bimbingan_m->get_all_bimbingan($this->session->userdata('id'));
			$count = 0;
		}
